...
public class ConnectionDao {
    ...
    public void createDb() throws SQLException {
        String SQL = "create table if not exists teachers" +
                ...
                "\n" +
                "\n" +
                "create table if not exists courseclients\n" +
                "(\n" +
    ...
    public void addClient(Client client) throws SQLException {
        String SQL = "INSERT INTO clients(name,type) VALUES(?,?)";
        pstmt = connection.prepareStatement(SQL);
    ...
    public void updateClient(int id, Client client) throws SQLException {
        String SQL = "UPDATE clients SET name = ?, type = ? WHERE id = ?";
        pstmt = connection.prepareStatement(SQL);
        pstmt.setString(1, client.getName());
        pstmt.setString(2, client.getType());
        pstmt.setInt(3, id);
    ...
    public boolean getClientByName(String name) throws SQLException {
        rs = statement.executeQuery("SELECT id FROM clients WHERE name = '" + name + "'");
        if (rs.next()) {
            return true;
        }
    ...
    public int getClientById(int id) throws SQLException {
        rs = statement.executeQuery("SELECT id FROM clients WHERE id = '" + id + "'");
        if (rs.next()) {
            return rs.getInt(1);
        }
        System.out.println("Такого клиента нету");
    ...
    public void addCourse(Course course) throws SQLException {
        if(!getCourseByName(course.getName())) {
            String SQL = "INSERT INTO courses (coursename, payment, month, teachername) VALUES(?, ?, ?, ?)";
            pstmt = connection.prepareStatement(SQL);
    ...
    public void updateCourse(Course course) throws SQLException {
        if(getCourseByName(course.getName())) {
            String SQL = "UPDATE courses SET payment = ?, month = ? WHERE coursename = ?";
            pstmt = connection.prepareStatement(SQL);
    ...
    public void deleteCourseByName(String name) throws SQLException {
        if(getCourseByName(name)) {
            statement.executeUpdate("DELETE FROM courses WHERE coursename = '" + name + "'");
        }
    }
    public boolean getCourseByName(String coursename) throws SQLException {
        rs = statement.executeQuery("SELECT coursename FROM courses WHERE coursename = '" + coursename + "'");
        if (rs.next()) {
            return true;
        }
    ...
    public Course getCourseObjByName(String coursename) throws SQLException {
        rs = statement.executeQuery("SELECT * FROM courses WHERE coursename = '" + coursename + "'");
        if (rs.next()) {
    ...
    public void getCourseMembers(String coursename) throws SQLException {
        rs = statement.executeQuery("select name, course_id, teachername from courseclients inner join clients on client_id = id where course_id = '" + coursename + "'");
    ...
    public void getMemberCourses(String name) throws SQLException {
        rs = statement.executeQuery("select name, course_id, teachername from courseclients inner join clients on client_id = id where client_id = '" + getClientIdByName(name) + "'");
        System.out.printf("%-30.30s %-30.30s %-30.30s%n", "Имя клиента", "Название курса", "Имя преподавателя");
        while (rs.next()) {
            System.out.printf("%-30.30s %-30.30s %-30.30s%n", rs.getString("name"), rs.getString("course_id"), rs.getString("teachername"));
        }
    ...
    public Teacher getTeacherByName(String name) throws SQLException {
        rs = statement.executeQuery("SELECT name FROM teachers WHERE name = '" + name + "'");
        if (rs.next()) {
    ...
    public void deleteTeacher(String teacherName) throws SQLException {
        statement.executeUpdate("DELETE FROM teachers WHERE name = '" + teacherName + "'");
    }
    ...
    public String getTeacherByCourse(String coursename) throws SQLException {
        rs = statement.executeQuery("SELECT teachername FROM courses WHERE coursename = '" + coursename + "'");
        if (rs.next()) {
    ...
    public void addToCourse(String name, String coursename, String teacherName) throws SQLException {
        if (getClientByName(name) || getCourseByName(coursename)) {
            String SQL = "INSERT INTO courseclients(client_id, course_id, teachername) VALUES(?, ?, ?)";
            pstmt = connection.prepareStatement(SQL);
    ...
    public void deleteFromCourse(String name, String coursename) throws SQLException {
        statement.executeUpdate("DELETE FROM courseclients WHERE client_id = '" + getClientIdByName(name) + "' and  course_id = '" + coursename + "'");
    }

    public boolean getCourseMember(String name, String coursename) throws SQLException {
        rs = statement.executeQuery("SELECT * FROM courseclients WHERE client_id = '" + getClientIdByName(name) + "' and course_id = '" + coursename +"'");
        if (rs.next()) {
            return true;
        }
